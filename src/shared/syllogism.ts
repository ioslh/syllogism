...
export const validSyllogisms = [
  { form: 'AAA-1', name: 'Barbara' },
  { form: 'EAE-1', name: 'Celarent' },
  { form: 'AII-1', name: 'Darii' },
  { form: 'EIO-1', name: 'Ferio' },
  { form: 'AEE-2', name: 'Camestres' },
  { form: 'EAE-2', name: 'Cesare' },
  { form: 'AOO-2', name: 'Baroko' },
  { form: 'EIO-2', name: 'Festino' },
  { form: 'AII-3', name: 'Datisi' },
  { form: 'IAI-3', name: 'Disamis' },
  { form: 'EIO-3', name: 'Ferison' },
  { form: 'OAO-3', name: 'Bokardo' },
  { form: 'AEE-4', name: 'Camenes' },
  { form: 'IAI-4', name: 'Dimaris' },
  { form: 'EIO-4', name: 'Fresison' },
]
...
// 返回 true 表明存在谬误
const checkTwoNegativePremises = (mood: Mood, _figure: Figure) => {
  const majorPremise = mood[0] as PropositionType
  const minorPremise = mood[1] as PropositionType
...
// 返回 true 表明存在谬误
const checkNegativeConclusion = (mood: Mood, _figure: Figure) => {
  const majorPremise = mood[0] as PropositionType
  const minorPremise = mood[1] as PropositionType
  const conclusion = mood[2] as PropositionType
...
// 返回 true 表明违反规则
const checkExistentialFallacy = (mood: Mood, _figure: Figure) => {
  const majorPremise = mood[0] as PropositionType
  const minorPremise = mood[1] as PropositionType
  const conclusion = mood[2] as PropositionType
...
export const argumentAssert = [
  { fn: checkUndistributeMiddle, key: 'UNDISTRIBUTE_MIDDLE' },
  { fn: checkIllicitProcess, key: 'ILLICIT_PROCESS' },
  { fn: checkTwoNegativePremises, key: 'TWO_NEGATIVE' },
  { fn: checkNegativeConclusion, key: 'NEGATIVE_CONCLUSION' },
  { fn: checkExistentialFallacy, key: 'EXISTENTIAL' },
]

export const getFallacyExplains = (lang: string): Record<string, { name: string; detail: string }> => {
  const isZh = lang === 'zh'
  return {
    UNDISTRIBUTE_MIDDLE: {
      name: isZh ? '中项不周延谬误' : 'fallacy of the undistributed middle',
      detail: isZh
    ...
    ILLICIT_PROCESS: {
      name: isZh ? '不当周延谬误' : 'fallacy of illicit process',
      detail: isZh
        ? '在结论中周延的项在前提中也必须周延。述及一个类的全部对象，比述及其中某些对象要断定更多。所以如果三段论前提中不周延的项在结论中周延，也就是结论断定了比前提更多的东西，结论超出了前提。'
        : `To refer to all members of a class is to say more about that class than is said when only some of its members are referred to. Therefore, when the conclusion of a syllogism distributes a term that was undistributed in the premises, it says more about that term than the premises did.`,
    },
    TWO_NEGATIVE: {
      name: isZh ? '排斥前提谬误' : 'fallacy of exclusive premises',
      detail: isZh
        ? '避免出现两个否定前提。任何否定命题都否认类的包含关系，断定一个类的部分或者全部被排除在另一类的全体之外。但是，有两个断定这种排斥性的前提不能得出结论中的联系，因此不可能是有效的论证。'
        : `Any negative proposition (E or O) denies class inclusion; it asserts that some or all of one class is excluded from the whole of the other class. Two premises asserting such exclusion cannot yield the linkage that the conclusion asserts, and therefore cannot yield a valid argument.`,
    },
    NEGATIVE_CONCLUSION: {
      name: isZh ? '从否定推肯定谬误' : 'fallacy of drawing an affirmative conclusion from a negative premise',
      detail: isZh
    ...
    EXISTENTIAL: {
      name: isZh ? '存在谬误' : 'existential fallacy',
      detail: isZh
        ? '在直言三段论的布尔解释中，全程命题没有存在含义，但特称命题却有存在含义。避免从没有存在含义的前提得出有存在含义的结论。'
        : `In the Boolean interpretation of categorical propositions, universal propositions (A and E) have no existential import, but particular propositions (I and O) do have such import.`,
    },
  }
}
